...
int main(int argc, char *argv[])
{
	...
	t0 = get_wtime();
	int num_threads = 16;
    #pragma omp parallel num_threads(num_threads)  /*Starts Parallelization and Creates Threads with num_threads(int) parameter, we use firstprivate to initialize our values*/
    {
         #pragma omp single nowait /*The code below can be processed by any thread without waiting for the other threads to end their processes*/
            for (trial = 0; trial < ntrials; trial++) {
				#pragma omp task firstprivate(fx, jj, i) /*Tasks initialization to calculate the function*/
				{
	...
	printf("Total number of function evaluations = %ld\n", funevals * num_threads); /*Multiply with num_threads to find the number of evaluations (it is shared in n threads)*/
